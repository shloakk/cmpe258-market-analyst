...
Usage:
    ...
    python eval/evaluator.py --models gemini llama qwen --write-results
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        choices=["gemini", "llama", "qwen", "nemotron", "gpt"],
        default="gemini",
        help=(
            "LLM used by Mapper and Critic for a single-model eval. Defaults "
            "to 'gemini' because the local demo path uses Google AI Studio. "
            "Use --model llama (Groq) or --model qwen/--model nemotron "
            "(OpenRouter) for free open-weight comparison, or --model gpt "
            "(OpenAI, paid) when OPENAI_API_KEY is set."
        ),
    )
    parser.add_argument(
        "--models",
        ...
        help=(
            ...
            "eval/results/latest.json. Example: --models gemini llama gpt"
        ),
    )
    parser.add_argument(
        "--cache-dir",
        ...
        help=(
            ...
            "Results are written on cache miss and read on cache hit, so "
            "repeated runs (e.g. in CI) never re-call the LLM. Commit the "
            "cache directory to avoid hitting daily token limits across "
            "runs. Example: eval/cache/gemini"
        ),
    )
    parser.add_argument(
        "--cache-root",
